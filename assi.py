    ...
    while len(seq_branches) >= 1:
        ...
        for i, formula in enumerate(seq_current.rhs):
            ...
            if formula.decl().name() == "=>":
                left = formula.arg(0)
                right = formula.arg(1)
                new_rhs = seq_current.rhs[:i] + seq_current.rhs[i+1:]
                new_seq = sequent(seq_current.lhs + [left], new_rhs + [right])
        ...
        for i, formula in enumerate(seq_current.lhs):
            ...
            if formula.decl().name() == "and":
                f1 = formula.arg(0)
                f2 = formula.arg(1)
                new_lhs = seq_current.lhs[:i] + seq_current.lhs[i+1:] + [f1, f2]
        ...
        for i, formula in enumerate(seq_current.rhs):
            ...
            if formula.decl().name() == "or":
                f1 = formula.arg(0)
                f2 = formula.arg(1)
                new_rhs = seq_current.rhs[:i] + seq_current.rhs[i+1:] + [f1, f2]
        ...
        for i, formula in enumerate(seq_current.lhs):
            ...
            if (formula.decl().name() == "not"):
                f1 = formula.arg(0)
                new_lhs = seq_current.lhs[:i] + seq_current.lhs[i+1:]
                new_seq = sequent(new_lhs, seq_current.rhs + [f1])
                seq_branches.append(new_seq)
                steps_taken += 1
                rule_applied = True
                break
        ...
        for i, formula in enumerate(seq_current.rhs):
            ...
            if (formula.decl().name() == "not"):
                f1 = formula.arg(0)
                new_rhs = seq_current.rhs[:i] + seq_current.rhs[i+1:]
                new_seq = sequent(seq_current.lhs + [f1], new_rhs)
                seq_branches.append(new_seq)
                steps_taken += 1
                rule_applied = True
                break
        ...
        for i, formula in enumerate(seq_current.rhs):
            ...
            if formula.is_forall():
                ...
                new_rhs = seq_current.rhs[:i] + seq_current.rhs[i+1:] + [instantiated]
                new_seq = sequent(seq_current.lhs, new_rhs)
                seq_branches.append(new_seq)
                steps_taken += 1
                rule_applied = True
        ...
        for i, formula in enumerate(seq_current.lhs):
            ...
            if formula.is_exists():
                ...
                fresh_vars = [fresh_const() for _ in range(variableCount)]
                instantiated = substitute_vars(body, *reversed(fresh_vars))
                new_lhs = seq_current.lhs[:i] + seq_current.lhs[i+1:] + [instantiated]
                new_seq = sequent(new_lhs, seq_current.rhs)
                seq_branches.append(new_seq)
                steps_taken += 1
                rule_applied = True
        ...
        for i, formula in enumerate(seq_current.lhs):
            ...
            if formula.decl().name() == "or":
                f0 = formula.arg(0)
                f1 = formula.arg(1)
                new_lhs1 = seq_current.lhs[:i] + seq_current.lhs[i+1:] + [f0]
                new_lhs2 = seq_current.lhs[:i] + seq_current.lhs[i+1:] + [f1]
                new_seq1 = sequent(new_lhs1, seq_current.rhs)
                new_seq2 = sequent(new_lhs2, seq_current.rhs)
        ...
        for i, formula in enumerate(seq_current.rhs):
            ...
            if formula.decl().name() == "and":
                f0 = formula.arg(0)
                f1 = formula.arg(1)
                new_rhs1 = seq_current.rhs[:i] + seq_current.rhs[i+1:] + [f0]
                new_rhs2 = seq_current.rhs[:i] + seq_current.rhs[i+1:] + [f1]
                new_seq1 = sequent(seq_current.lhs, new_rhs1)
                new_seq2 = sequent(seq_current.lhs, new_rhs2)
                seq_branches.append(new_seq1)
                seq_branches.append(new_seq2)
                steps_taken += 1
                rule_applied = True
        ...
        for i, formula in enumerate(seq_current.lhs):
            ...
            if formula.decl().name() == "=>":
                f0 = formula.arg(0)
                f1 = formula.arg(1)
                seq1_new_rhs = seq_current.rhs + [f0]
                seq1_new_lhs = seq_current.lhs[:i] + seq_current.lhs[i+1:]
                seq2_new_rhs = seq_current.rhs
                seq2_new_lhs = seq_current.lhs[:i] + seq_current.lhs[i+1:] + [f1]
