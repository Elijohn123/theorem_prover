...
class FormulaHeap:
    ...
    def push(self, formula):
        # negative length so heapq (min-heap) gives us longest first
        heapq.heappush(self._heap, (-len(str(formula)), id(formula), formula))

    def __iter__(self):
        # yield formulas in longest-first order
    ...
    def remove_at(self, idx):
        lst = list(self)
        target = lst[idx]
        self._heap = [item for item in self._heap if item[2] is not target]
        heapq.heapify(self._heap)
        return target

    def slice_except(self, idx):
        """Return a new FormulaHeap with the formula at idx removed."""
        lst = list(self)
        target = lst[idx]
        new = FormulaHeap()
    ...
    def plus(self, *formulas):
        """Return a new FormulaHeap with extra formulas added."""
        new = self.copy()
        for f in formulas:
            new.push(f)
        return new
...
def apply_andL_rule(seq):
    ...
    for i, formula in enumerate(seq.lhs): 
        if is_app(formula) and get_formula_name(formula) == "and":
            f1 = formula.arg(0)
            f2 = formula.arg(1)
            new_lhs = seq.lhs.slice_except(i).plus(f1, f2)
            print("∧L rule applied") if TEST_MODE else None
            steps_taken += 1
...
def apply_orR_rule(seq):
    ...
    for i, formula in enumerate(seq.rhs):
        if is_app(formula) and get_formula_name(formula) == "or":
            f1 = formula.arg(0)
            f2 = formula.arg(1)
            new_rhs = seq.rhs.slice_except(i).plus(f1, f2)
            print("∨R rule applied") if TEST_MODE else None
            steps_taken += 1
...
def apply_implication_R_rule(seq):
    ...
    for i, formula in enumerate(seq.rhs): 
        if is_app(formula) and get_formula_name(formula) == "=>":
            left = formula.arg(0)
            right = formula.arg(1)
            new_rhs = seq.rhs.slice_except(i).plus(right)
            new_lhs = seq.lhs.plus(left)
            print("→R rule applied") if TEST_MODE else None
            steps_taken += 1
            return sequent(new_lhs, new_rhs)
    return False
...
def apply_negationR_rule(seq):
    ...
    for i, formula in enumerate(seq.rhs):
        if is_app(formula) and get_formula_name(formula) == "not":
            f1 = formula.arg(0)
            new_rhs = seq.rhs.slice_except(i)
            new_lhs = seq.lhs.plus(f1)
            print("¬R rule applied") if TEST_MODE else None
            steps_taken += 1
            return sequent(new_lhs, new_rhs)
    return False
...
def apply_negationL_rule(seq):
    ...
    for i, formula in enumerate(seq.lhs):
        if is_app(formula) and get_formula_name(formula) == "not":
            f1 = formula.arg(0)
            new_lhs = seq.lhs.slice_except(i)
            new_rhs = seq.rhs.plus(f1)
            print("¬L rule applied") if TEST_MODE else None
            steps_taken += 1
            return sequent(new_lhs, new_rhs)
    return False
...
def apply_existsL_rule(seq):
    ...
    for i, formula in enumerate(seq.lhs):
        if is_quantifier(formula) and formula.is_exists():
            ...
            new_lhs = seq.lhs.slice_except(i).plus(instantiated)
...
def apply_forallR_rule(seq):
    ...
    for i, formula in enumerate(seq.rhs):
        if is_quantifier(formula) and formula.is_forall():
            ...
            fresh_vars = [fresh_const() for _ in range(variableCount)]
            instantiated = substitute_vars(body, *reversed(fresh_vars))
            new_rhs = seq.rhs.slice_except(i).plus(instantiated)
            print("∀R rule applied") if TEST_MODE else None
            steps_taken += 1
            return sequent(seq.lhs, new_rhs)
    return False
...
def apply_andR_rule(seq):
    ...
    for i, formula in enumerate(seq.rhs):
        if is_app(formula) and get_formula_name(formula) == "and":
            f0 = formula.arg(0)
            f1 = formula.arg(1)
            base_rhs = seq.rhs.slice_except(i)
            new_seq1 = sequent(seq.lhs, base_rhs.plus(f0))
            new_seq2 = sequent(seq.lhs, base_rhs.plus(f1))
            print("∧R rule applied") if TEST_MODE else None
            steps_taken += 1
...
def apply_orL_rule(seq):
    ...
    for i, formula in enumerate(seq.lhs):
        if is_app(formula) and get_formula_name(formula) == "or":
            f0 = formula.arg(0)
            f1 = formula.arg(1)
            base_lhs = seq.lhs.slice_except(i)
            new_seq1 = sequent(base_lhs.plus(f0), seq.rhs)
            new_seq2 = sequent(base_lhs.plus(f1), seq.rhs)
            print("∨L rule applied") if TEST_MODE else None
            steps_taken += 1
            return (new_seq1, new_seq2)
    return False
...
def apply_implication_L_rule(seq):
    ...
    for i, formula in enumerate(seq.lhs):
        if is_app(formula) and get_formula_name(formula) == "=>":
            ...
            f1 = formula.arg(1)
            base_lhs = seq.lhs.slice_except(i)
            new_seq1 = sequent(base_lhs, seq.rhs.plus(f0))
            new_seq2 = sequent(base_lhs.plus(f1), seq.rhs)
            print("→L rule applied") if TEST_MODE else None
            steps_taken += 1
            return (new_seq1, new_seq2)
    ...
    with open(file_path, "r") as f:
        for line in f:
            ...
            try:
                ...
                if result == expected:
                    print(f"✅ Correct: {formula_str}") if TEST_MODE else None
                    correct += 1
                else:
                    print(f"❌ Wrong: {formula_str} | Expected: {expected}, Got: {result}") if TEST_MODE else None
                ...
                print(f"\n⚠️ Error processing line:") if TEST_MODE else None
